...
export class WindowLayoutManager {
  computeTile(index: number, total: number, windowSettings: Settings["window"]): Tile {
  ...
  async tileBrowserWindow(
    ...
    const tile = this.computeTile(index, total, settings.window);
...
const DEFAULT_WINDOW_LAYOUT_MANAGER = new WindowLayoutManager();

export function computeTile(index: number, total: number, windowSettings: Settings["window"]): Tile {
  return DEFAULT_WINDOW_LAYOUT_MANAGER.computeTile(index, total, windowSettings);
}
...
export async function tileBrowserWindow(
  ...
  await DEFAULT_WINDOW_LAYOUT_MANAGER.tileBrowserWindow(context, index, total, settings);
}
